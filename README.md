...
- **Phase 2 (planned):** more tools (web search, note editing/deletion).
...
# API_KEY (any random string you choose - this protects your own /chat
# endpoint, unrelated to the Anthropic key; generate one with
...
Run tests (no real API key needed - the LLM call is mocked):
...
**Persistence** for the `save_note`/`get_notes` tool uses SQLite directly via `aiosqlite` (no ORM - one table doesn't justify one). On Render's free tier this resets on redeploy since there's no attached persistent volume.
...
Calendar integration, web search, note editing/deletion, persistent conversation history across restarts, vector/long-term memory, voice I/O, frontend UI, multi-user auth.
